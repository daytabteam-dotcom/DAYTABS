...
import { Wand2, CheckCircle2, AlertTriangle, XCircle, ChevronDown, ChevronUp, Lightbulb, Zap, Upload, Film, Brain, MonitorPlay, Volume2, FileDown, Lock } from "lucide-react";
...
function FillerWordCard({ metric }: { metric: any }) {
  ...
  if (metric.assessment) {
    ...
    common.forEach(fw => { if (metric.assessment.toLowerCase().includes(fw) && !fillerTokens.includes(fw)) fillerTokens.push(fw); });
  }
  if (metric.words?.length) metric.words.forEach((w: string) => { if (!fillerTokens.includes(w)) fillerTokens.push(w); });
  return (
    <div className={`bg-background/60 rounded-xl p-4 border ${borderColor} col-span-2 hover:border-primary/20 transition-all`}>
      <div className="flex justify-between items-start mb-3">
        <div className="flex items-center gap-2"><Volume2 className={`w-4 h-4 ${iconColor}`} /><p className="text-xs text-white/40 uppercase tracking-wider">Filler Words</p></div>
        <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold border ${badgeColor}`}><Icon className="w-3.5 h-3.5" />{metric.level}</div>
      </div>
      <div className="flex items-end gap-4 mb-3">
        <div><span className="text-3xl font-bold font-mono">{numVal}</span><span className="text-xs text-white/40 ml-1">/ 100</span></div>
        {metric.count != null && <span className="text-sm text-white/40 mb-1">{metric.count} instance{metric.count !== 1 ? "s" : ""} detected</span>}
      </div>
      ...
      {fillerTokens.length > 0 && (
        <div className="mb-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {fillerTokens.map((fw, i) => <span key={i} className="px-2.5 py-1 bg-red-400/10 border border-red-400/20 text-red-300 rounded-lg text-xs font-mono font-medium">"{fw}"</span>)}
          </div>
        </div>
      )}
      {metric.suggestions?.length > 0 && (
        <div className="space-y-1">{metric.suggestions.map((s: string, i: number) => <p key={i} className="text-xs text-primary/80">→ {s}</p>)}</div>
      )}
    </div>
  );
}
...
function UploadZone({ onFile, isPending, maxSizeLabel }: { onFile: (f: File) => void; isPending: boolean; maxSizeLabel?: string }) {
  ...
  return (
    <div {...getRootProps()} className={`relative overflow-hidden rounded-2xl cursor-pointer transition-all duration-300 border-2 ${isDragActive ? "border-primary bg-primary/10 scale-[1.01]" : "border-white/10 hover:border-primary/40 bg-primary/3 hover:bg-primary/6"} ${file ? "min-h-[200px] p-2" : "p-10"}`}>
      <input {...getInputProps()} />
      {file && preview ? (
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="relative">
            <div className="w-20 h-20 rounded-full bg-primary/15 border border-primary/20 flex items-center justify-center"><Brain className="w-9 h-9 text-primary/70" /></div>
            <div className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-primary/30 border border-primary/40 flex items-center justify-center"><Upload className="w-3.5 h-3.5 text-primary" /></div>
          </div>
...
export default function PreEditTab({ onDataReady, onDataReset, onRegisterExport }: TabProps) {
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto space-y-6">
        ...
        {uploadLimit !== -1 && (
          <div className="flex items-center justify-between px-4 py-2.5 rounded-xl bg-white/3 border border-white/8">
            ...
            <span className={`text-xs font-semibold ${uploadsRemaining === 0 ? "text-red-400" : "text-violet-400"}`}>
        ...
        {limitReached ? (
          <div className="text-center py-8 space-y-3">
            ...
            <p className="text-white/60 text-sm">You've used all {uploadLimit} pre-edit uploads for this month.</p>
            <LockedBadge />
          </div>
        ) : (
          <>
  ...
  // Free plan: only first 4 visual metrics unlocked, rest blurred
  const freeVisualKeys = ["lighting", "brightness", "contrast", "sharpness"];
  const lockedVisualKeys = ["stability", "colorBalance", "background", "framing"];
  const audioKeys = ["audioVolume", "audioClarity", "backgroundNoise"];
  const freeAudioKeys = ["audioVolume"];
  const lockedAudioKeys = ["audioClarity", "backgroundNoise"];
      ...
      <motion.div ref={ref} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-10">
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {freeVisualKeys.map(k => <MetricCard key={k} title={k} metric={quality?.[k]} />)}
            {isFree
              ? lockedVisualKeys.map(k => (
                  <LockedContent key={k} locked className="rounded-xl">
                    <MetricCard title={k} metric={quality?.[k] ?? { numeric: 75, level: "Good", assessment: "Upgrade to see this metric." }} />
                  </LockedContent>
                ))
              : lockedVisualKeys.map(k => <MetricCard key={k} title={k} metric={quality?.[k]} />)
            }
          </div>
        </section>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {freeAudioKeys.map(k => <MetricCard key={k} title={k} metric={quality?.[k]} />)}
            {isFree
              ? lockedAudioKeys.map(k => (
                  <LockedContent key={k} locked className="rounded-xl">
                    <MetricCard title={k} metric={quality?.[k] ?? { numeric: 80, level: "Good", assessment: "Upgrade to see this metric." }} />
                  </LockedContent>
                ))
              : lockedAudioKeys.map(k => <MetricCard key={k} title={k} metric={quality?.[k]} />)
            }
            {quality?.fillerWords && (
              isFree
                ? <LockedContent locked label="Filler word analysis requires Premium" className="col-span-2 rounded-xl">
                    <FillerWordCard metric={quality.fillerWords} />
                  </LockedContent>
                : <FillerWordCard metric={quality.fillerWords} />
            )}
          </div>
                ...
                <div className="space-y-2">
                  {scriptFeedback.hookSuggestions.map((s: string, i: number) => (
                    i === 0 || !isFree
                      ...
                      : <LockedContent key={i} locked label="Upgrade to see more hooks">
                          <div className="flex gap-3 p-3 bg-yellow-400/5 border border-yellow-400/15 rounded-xl">
                            <span className="text-yellow-400 font-bold shrink-0">{i + 1}.</span>
                            <p className="text-sm text-white/80">{s}</p>
                          </div>
                        </LockedContent>
                ...
                <div className="space-y-3">
                  {scriptFeedback.weakSections.map((ws: any, i: number) => (
                    i < 2 || !isFree
                      ...
                      : <LockedContent key={i} locked label="Upgrade to see all weak sections">
                          <div className="p-4 bg-red-400/5 border border-red-400/15 rounded-xl space-y-2">
                            <p className="text-xs text-red-300 font-medium">{ws.reason}</p>
                            <p className="text-sm text-white/50 line-through">"{ws.text}"</p>
                          </div>
                        </LockedContent>
