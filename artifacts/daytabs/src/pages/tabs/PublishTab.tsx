  ...
  if (!jobId) {
    return (
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto space-y-6">
        ...
        <button
          onClick={() => selectedFile && upload({ file: selectedFile, options: { mode: "publish", platform, translateSubtitles: translateSubs, subtitleLanguage: translateSubs ? subLang : undefined } }, { onSuccess: (d) => { setJobId(d.jobId); toast({ title: "Optimizing for publishing…" }); } })}
