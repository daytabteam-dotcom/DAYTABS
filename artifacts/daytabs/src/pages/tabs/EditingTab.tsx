...
import { Scissors, Clock, Zap, Film, Lightbulb, MessageSquare, Upload, FileDown, Lock } from "lucide-react";
...
function UploadZone({ onFile, isPending, maxSizeLabel }: { onFile: (f: File) => void; isPending: boolean; maxSizeLabel?: string }) {
  ...
  return (
    <div {...getRootProps()} className={`relative overflow-hidden rounded-2xl cursor-pointer transition-all duration-300 border-2 ${isDragActive ? "border-yellow-400 bg-yellow-400/10 scale-[1.01]" : "border-white/10 hover:border-yellow-400/40 bg-yellow-400/3 hover:bg-yellow-400/6"} ${file ? "min-h-[200px] p-2" : "p-10"}`}>
      <input {...getInputProps()} />
      {file && preview ? (
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="relative">
            <div className="w-20 h-20 rounded-full bg-yellow-400/15 border border-yellow-400/20 flex items-center justify-center"><Scissors className="w-9 h-9 text-yellow-400/70" /></div>
            <div className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-yellow-400/30 border border-yellow-400/40 flex items-center justify-center"><Upload className="w-3.5 h-3.5 text-yellow-400" /></div>
          </div>
          ...
          <div className="px-5 py-2 bg-yellow-400/10 border border-yellow-400/20 rounded-full text-sm font-medium text-yellow-400/80">Browse Files</div>
        </div>
      )}
    </div>
  );
...
export default function EditingTab({ onDataReady, onDataReset, onRegisterExport }: TabProps) {
  ...
  const { uploadsRemaining, uploadUsed, uploadLimit } = getModeLimits("editing");
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto space-y-6">
        ...
        {uploadLimit !== -1 && (
          <div className="flex items-center justify-between px-4 py-2.5 rounded-xl bg-white/3 border border-white/8">
            ...
            <span className={`text-xs font-semibold ${uploadsRemaining === 0 ? "text-red-400" : "text-yellow-400"}`}>
        ...
        {limitReached ? (
          <div className="text-center py-8 space-y-3">
            ...
            <p className="text-white/60 text-sm">You've used all {uploadLimit} editing uploads for this month.</p>
            <LockedBadge />
          </div>
        ) : (
          <>
            ...
            <button
              onClick={() => {
                ...
                upload({ file: selectedFile, options: { mode: "editing", platform: "youtube_long" } }, { onSuccess: (d) => { setJobId(d.jobId); toast({ title: "Finding editing points…" }); } });
              }}
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm font-medium bg-yellow-400/10 hover:bg-yellow-400/20 border border-yellow-400/25 text-yellow-300 rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isPdfExporting ? <><div className="w-3.5 h-3.5 border-2 border-yellow-400/30 border-t-yellow-400 rounded-full animate-spin" />Exporting…</> : <><FileDown className="w-3.5 h-3.5" />Download PDF</>}
            </button>
        ...
        {results.hooks?.length > 0 && (
          <div className="glass-card rounded-2xl p-6 border border-yellow-400/20">
            <div className="flex items-center gap-2 mb-4">
              <Zap className="w-4 h-4 text-yellow-400" /><h3 className="font-semibold">Strong Hooks</h3>
              <span className="ml-auto text-xs bg-yellow-400/10 text-yellow-400 px-2 py-0.5 rounded-full">{results.hooks.length}</span>
            </div>
            <div className="space-y-3">
              {results.hooks.map((h: any, i: number) => (
                i === 0 || !isFree
                  ...
                  : <LockedContent key={i} locked label="Upgrade to see all hooks">
      ...
      {results.shortVideos?.length > 0 && (
        isFree
          ? <LockedContent locked label="Short-form segment detection requires Premium" className="rounded-2xl">
          ...
          <div className="space-y-2">
            {results.editingSuggestions.map((tip: string, i: number) => (
              i < 2 || !isFree
                ...
                : <LockedContent key={i} locked label="Upgrade to see all editing tips">
                    <div className="flex items-start gap-3 p-3 bg-blue-400/5 border border-blue-400/10 rounded-xl">
                      <span className="text-blue-400 font-bold shrink-0 text-sm">{i + 1}.</span>
                      <p className="text-sm text-white/70">{tip}</p>
                    </div>
                  </LockedContent>
          ...
          <div className="space-y-1.5 max-h-72 overflow-y-auto pr-1">
            {results.transcript.map((seg: any, i: number) => (
              i < 3 || !isFree
                ...
                : i === 3 && isFree
                  ? <LockedContent key={i} locked label="Upgrade to see full transcript" className="rounded-xl">
                      <div className="flex gap-3 py-1.5">
                        <span className="text-xs font-mono text-yellow-400/60 shrink-0 w-12">{seg.time}</span>
                        <p className="text-sm text-white/70">…more transcript lines available with Premium</p>
                      </div>
                    </LockedContent>
                  : null
            ))}
